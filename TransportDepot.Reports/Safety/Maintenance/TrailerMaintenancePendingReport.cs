...
{
  class TrailerMaintenancePendingReport
  {
    private string[,] _reportData = null;
    private readonly double[] _columnWidths = { 2, 6, 6 };
    private Dictionary<Trailer, IEnumerable<DateTime>> _data;
    ...
    public TrailerMaintenancePendingReport(Dictionary<Trailer, IEnumerable<DateTime>> data, DateTime from, DateTime to)
    {
    ...
    {
      var maintenanceCount = this._data.Values.Select(v => v.Count()).Sum();
      int arrayHeight = maintenanceCount + this._data.Keys.Count * 2 + 3;
